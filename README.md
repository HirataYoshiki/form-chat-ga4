...
4.  ブラウザで `http://localhost:3000` (または `vite.config.ts` で設定したポート) を開きます。
    管理画面は `/admin/tenants` パスでアクセスできます (例: `http://localhost:3000/admin/tenants`)。

開発サーバーは、バックエンドAPI (デフォルトで `http://localhost:8000` で実行されている想定) へのリクエストをプロキシするように設定されています (`vite.config.ts` を参照)。

### ディレクトリ構成のポイント
- `frontend/src/components/`: 主要なページコンポーネント (e.g., `TenantListPage.tsx`, `TenantCreatePage.tsx`)
- `frontend/src/api.ts`: バックエンドAPIと通信するための `axios` クライアントインスタンス
- `frontend/src/App.tsx`: アプリケーション全体のルーティング設定

### 認証に関する注意点
現在のフロントエンド実装では、バックエンドAPIへのリクエストに認証トークンを自動的に含める仕組みは組み込まれていません。
テナント管理APIはスーパーユーザー権限を必要とするため、実際のAPI操作 (特にデータの作成・更新・削除) を行うには、以下のいずれかの対応が必要です:
- バックエンドの認証チェックを一時的に無効化する (開発時)。
- 有効なスーパーユーザーアカウントでログインして取得したJWTトークンを、ブラウザの開発者ツールなどを用いて手動で `apiClient` のリクエストヘッダーに設定する。
- 今後、フロントエンドにログイン機能を実装し、トークンを適切に管理する。
