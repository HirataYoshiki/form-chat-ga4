...
-   **`GET /api/v1/tenants/{tenant_id}/rag_files`**:
    ...
    -   **認証**: スーパーユーザー（またはテナント管理者）。
    -   **レスポンス**: ファイルリスト（ファイル名、アップロード日時、ステータスなど）。
-   **`GET /api/v1/tenants/{tenant_id}/rag_files/{file_id}`**:
    -   **目的**: 指定されたファイルの詳細情報や処理ステータスを取得します。
    -   **認証**: スーパーユーザー（またはテナント管理者）。
    -   **レスポンス**: ファイル詳細情報。
-   **`DELETE /api/v1/tenants/{tenant_id}/rag_files/{file_id}`**:
    -   **目的**: 指定されたファイルをRAGシステムから削除します（メタデータおよび関連インデックスの削除）。
    -   **認証**: スーパーユーザー（またはテナント管理者）。
    -   **レスポンス**: 成功メッセージ。
